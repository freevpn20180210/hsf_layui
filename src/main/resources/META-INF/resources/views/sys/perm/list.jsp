    ...
    layui.config({
        ...
        var DTreeNode = dtree.render({
            ...
            success: function (res, $obj, first) {
                $('#add').click(function () {
                    layuiNewWindow_normalSubmit('添加', '<%=path%>/views/sys/perm/saveUpdate.jsp', '600px', '500px')
                })
                layer.close(index)
            },
            skin: "layui",
            ...
            menubarFun: {
                remove: function (checkbarNodes) { // 必须将该方法实现了，节点才会真正的从后台删除哦
                    ...
                    $.post('<%=path%>/perm/del', {ids: ids.toString()}, function (rs) {
                        if (rs.ok) {
                            myRefresh()
                        } else {
                            layer.msg(rs.msg)
        ...
        dtree.on("node('dtree')", function (obj) {
            ...
            //获取节点信息
            $.post('<%=path%>/perm/getInfoById', {id: param.nodeId}, function (rs) {
                var po = rs.po
                $('#submit').show()
                form.val('form', {
                    'id': po.id,
